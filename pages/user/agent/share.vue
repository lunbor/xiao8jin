<template>
	<view>
		<view class="agent_user_bg">
			<view class="agent_total_user">共<text>{{agentCount}}</text>人</view>
		</view>
	...
	export default {
		...
		methods: {
			getAgentShareData(){
				...
				try {
					this.$http.post(this.websiteUrl+'/api/agent/getAgentShareData',{openid,sessionKey,page},(res) => {
						if(res.data.code==1){
							if(res.data.result.list.length>=1){
								this.list = this.list.concat(res.data.result.list);
								this.agentCount = res.data.result.agentCount;
								this.loadingType = 0;
