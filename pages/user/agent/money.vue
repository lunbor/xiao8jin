					...
					<view class="list-item" v-for="(item,index) in list" :key="index">
						<view class="uni-flex uni-row justify" >
							<view class="text left-text">
								<view class="text log-title">{{item.desc}}</view>
								<view class="text log-tail">{{item.change_time}}</view>
							</view>
							<view class="text right-text">
								<view class="text log-money">{{item.money}}</view>
								<view class="text log-balance">{{item.type}}</view>
							</view>
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			getAgentMoneyData(){
				...
				try {
					this.$http.post(this.websiteUrl+'/api/agent/getAgentMoneyData',{openid,sessionKey,page},(res) => {
						if(res.data.code==1){
...
<style>
	...
	.log-balance{font-size: 24upx; color: #999;}
